function [param, stat, Fit, x_vector, y_vector] = sigm_fit(x,y,fixed_params,initial_params,plot_flag)
...
y_vector = f(param(isnan(fixed_params)),x_vector);
Fit = f(param(isnan(fixed_params)),x);
% if plot_flag==1 
%     x_vector=min(x):(max(x)-min(x))/100:max(x);
