...
def set_peerNodes(target_name, op_status=0, isBroadcast=False):
	...
	if(op_status==1): 
		if(not isBroadcast):  
			VDFchain_client.add_node(target_address, json_node)
		else:
			VDFchain_client.add_node(list_address, json_node, True)
	if(op_status==2):
		if(not isBroadcast):
			VDFchain_client.remove_node(target_address, json_node)
		else:
			VDFchain_client.remove_node(list_address, json_node, True)
...
def Epoch_validator(target_address, op_status, tx_size, tx_count, phase_delay=BOUNDED_TIME):
	...
	## S2: start mining 
	start_time=time.time()   
	VDFchain_client.start_mining(target_address, True)
	exec_time=time.time()-start_time
	ls_time_exec.append(format(exec_time*1000, '.3f'))
	...
	start_time=time.time() 
	VDFchain_client.start_voting(target_address, True)
	exec_time=time.time()-start_time
	ls_time_exec.append(format(exec_time*1000, '.3f'))
...
def Epoch_randomshare(phase_delay=BOUNDED_TIME):
	...
	# 	VDFchain_client.create_randshare(json_node['node_url'])
	VDFchain_client.create_randshare(peer_nodes.get_nodelist(), True)
	exec_time=time.time()-start_time
	ls_time_exec.append(format(exec_time*1000, '.3f'))
	...
	# for peer_node in list(peer_nodes.get_nodelist()):
	# 	json_node = TypesUtil.string_to_json(peer_node)
	# 	VDFchain_client.cache_fetch_share(json_node['node_url'])
	VDFchain_client.fetch_randshare(peer_nodes.get_nodelist(), True)
	exec_time=time.time()-start_time
	ls_time_exec.append(format(exec_time*1000, '.3f'))
	...
	start_time=time.time()
	VDFchain_client.verify_randshare(peer_nodes.get_nodelist(), True)
	exec_time=time.time()-start_time
	ls_time_exec.append(format(exec_time*1000, '.3f'))
	...
	for peer_node in list(peer_nodes.get_nodelist()):
		...
		VDFchain_client.vote_randshare(json_node['node_url'])
	...
	for peer_node in list(peer_nodes.get_nodelist()):
		json_node = TypesUtil.string_to_json(peer_node)
		VDFchain_client.cache_recovered_shares(json_node['node_url'])
	exec_time=time.time()-start_time
	ls_time_exec.append(format(exec_time*1000, '.3f'))
	...
	for peer_node in list(peer_nodes.get_nodelist()):
		json_node = TypesUtil.string_to_json(peer_node)
		ls_secret.append(VDFchain_client.recovered_shares(json_node['address']))
	exec_time=time.time()-start_time
	ls_time_exec.append(format(exec_time*1000, '.3f'))
	...
	start_time=time.time()
	VDFchain_client.new_random(ls_secret)
	exec_time=time.time()-start_time
	ls_time_exec.append(format(exec_time*1000, '.3f'))
...
if __name__ == "__main__":
	...
	if(test_func == 0):
		if(op_status == 1):
			...
			neighbors = VDFchain_client.get_neighbors(target_address)
			logger.info(neighbors)
		elif(op_status == 3):
			peers = VDFchain_client.get_peers(target_address)
			logger.info(peers)
		elif(op_status == 4):
			tasks = [VDFchain_client.get_peers_info(target_address)]
		...
		if(op_status == 1):
			...
			transactions = VDFchain_client.get_transactions(target_address)
